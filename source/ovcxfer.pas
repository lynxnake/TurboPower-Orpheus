...
begin
  ...
  try
    ...
    for I := 0 to xfrList.Count-1 do begin
      ...
      if C is TEdit then
        String(Pointer(P)^) := TEdit(C).Text
      else if C is TLabel then
        String(Pointer(P)^) := TLabel(C).Caption
      else if C is TPanel then
        String(Pointer(P)^) := TPanel(C).Caption
...
begin
  ...
  try
    ...
    for I := 0 to xfrList.Count-1 do begin
      C := TComponent(xfrList.Items[I]);
      if C is TEdit then
        TEdit(C).Text := String(Pointer(P)^)
      else if C is TLabel then
        TLabel(C).Caption := String(Pointer(P)^)
      else if C is TPanel then
        TPanel(C).Caption := String(Pointer(P)^)
      else if C is TMemo then begin
        Move(P^, ST, SizeOf(TStrings));
