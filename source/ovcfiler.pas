...
{$ENDIF}
    ...
    function GetUStringProperty(PropInfo : PExPropInfo): string;
    function GetWStringProperty(PropInfo : PExPropInfo): string;
    ...
    procedure SetWStringProperty(const S : string; PropInfo : PExPropInfo);
    procedure SetUStringProperty(const S : string; PropInfo : PExPropInfo);
...
begin
  ...
  for I := 0 to Count - 1 do
    with Items[I]^.PI do
      if (PropType = P^.PI.PropType) and (CompareText(string(Name), string(P^.PI.Name)) = 0) then begin
        Result := True;
        Break;
      end;
end;
    ...
    if Size > 0 then begin
      ...
      try
        ...
        for I := 0 to LCount-1 do begin
          ...
          if Prefix > '' then
            P^.PI.Name := ShortString(Prefix + '.' + string(P^.PI.Name));
          P^.AObject := AObject;
          FList.Add(P);
        end;
      finally
...
begin
  ...
  for I := 0 to Count - 1 do
    with PExPropInfo(FList[I])^.PI do
      if (CompareText(string(Name), AName) = 0) then begin
        Result := FList[I];
        Break;
      end;
end;
...
begin
  ...
  try
    with Saver do begin
      Prefix := Self.Prefix + string(PropInfo^.PI.Name);
...
function TOvcDataFiler.GetWStringProperty(PropInfo : PExPropInfo) : string;
begin
  Result := GetWideStrProp(PropInfo^.AObject, PPropInfo(PropInfo));
end;
...
procedure TOvcDataFiler.SetWStringProperty(const S : string; PropInfo : PExPropInfo);
begin
  SetWideStrProp(PropInfo^.AObject, PPropInfo(PropInfo), S);
end;
